...
func (cfg *Config) UpdateByFlags(ServAddr, BaseURL, FStorPath, PostgresURL *string) (bool, error) {
	...
	if *PostgresURL != "postgres://postgres:postgres@postgres:5432/praktikum?sslmode=disable" { // "host=localhost port=5432 user=postgres password=kornkorn dbname=mydb sslmode=disable" {
